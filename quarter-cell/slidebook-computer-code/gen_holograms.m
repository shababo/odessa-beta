...
target_base_fast.mode = 'Phase';
target_base_fast.pattern = 1040;
num_spots = length(tf_precomputed_target);
clear precomputed_target
N = 1000;
K = 4;
max_use = 6;
...
%             [max_pow,max_spot_ind] = max(spots_power);
            spot_power_ratios = round(power_map(20)./spots_power*10000);
    ...
    targets_cov = targets_cov_tmp;
    for k = 1:K
        fullF = fullF + sqrt(spot_power_ratios(k))*exp(1i*tf_precomputed_target(these_spots(k)).pattern);
    end
%     spot_ind = spots_key(these_spots(max_spot_ind),[1 2])/20 + 9;
...
figure;
subplot(121)
histogram(all_targets(:),.5:1:1000);
subplot(122)
histogram(all_distances(:))
targets = zeros(1089,good_stim);
for i = 1:good_stim
...
figure; histogram(targets_cov(:))
figure; histogram(pockels_ratio_refs_multi(:))

stim_count_map = zeros(33,33);
for i = 1:good_stim
    for k = 1:K
        inds = tf_stim_key(all_targets(i,k),[1 2])/10 + 17;
        stim_count_map(inds(1),inds(2)) = stim_count_map(inds(1),inds(2)) + 1;
    end
end
figure; imagesc(stim_count_map)

%%
tf_stim_key = zeros(500,3,3);
for i = 1:size(all_targets,1)
    for k = 1:size(all_targets,2)
        tf_stim_key(i,:,k) = tf_spots_key(all_targets(i,k),:);
    end
end
...
target.pattern = squeeze(tf_all_spots_phase(:,:,tf_spots_key(:,1) == 100 & tf_spots_key(:,2) == -100));
isTargetPatternReady = 1;
