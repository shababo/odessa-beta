...
if get(handles.test_pulse,'Value') && ~(2 == get(handles.Cell2_type_popup,'Value'));
    ...
    peak_capacitance = Min - Max;
    handles.data.ch2.series_r(sweep_counter)=(-4/peak_capacitance)*1000; % multiply by 1000 to corrent for nA to pA conversion


    % analyze holding current for cells 1 & 2
    avg1=mean(handles.data.ch2sweep((Fs*0.03):round(Fs*0.035))); % have to use round to keep second limit as integer
    handles.data.ch2.holding_i(sweep_counter)=avg1;

    % analyze input resistance for cells 1 & 2
    R1=4/(mean(handles.data.ch2sweep((Fs*.080):(Fs*.095)))-mean(handles.data.ch2sweep((Fs*.01):(Fs*.02))));
    handles.data.ch2.input_r(sweep_counter)=R1*1000;
else
    handles.data.ch2.series_r(sweep_counter) = NaN;
    handles.data.ch2.holding_i(sweep_counter) = NaN;
    handles.data.ch2.input_r(sweep_counter) = NaN;
end
